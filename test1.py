...
class DatabaseManager:
    ...
    def add_sample_data(self, con):
        ...
        for i in range(25):
            ...
            dob = date(random.randint(1980, 2005), random.randint(1, 12), random.randint(1, 28)).isoformat()
        ...
        for member_id in members:
            ...
            if scenario < 0.15:  # 15% expired
                start_date = today - relativedelta(months=duration+2)
                end_date = today - relativedelta(months=2)
            elif scenario < 0.30:  # 15% expiring within 7 days
                start_date = today - relativedelta(months=duration, days=-5)
                end_date = today + relativedelta(days=5)
            elif scenario < 0.50:  # 20% expiring within 30 days
                start_date = today - relativedelta(months=duration, days=-20)
            ...
            con.execute("""
                ...
                VALUES (?, ?, ?, ?, 'active', ?)
            """, (member_id, plan_id, start_date.isoformat(), end_date.isoformat(), notes))
